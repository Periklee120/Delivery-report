...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        try:
            ...
            for sheet_name, temp_df in sheets.items():
                ...
                # Έλεγχος αν το συγκεκριμένο tab
                # έχει τις απαραίτητες στήλες
                if all(
            ...
            st.error(
                f"❌ Πρόβλημα στην ανάγνωση του αρχείου "
                f"'{file.name}': {e}"
            )
    ...
    if not df_list:
        ...
        st.error(
            "❌ Δεν βρέθηκε κανένα tab με τις απαραίτητες "
            "στήλες: CneeAdd1, Cnee, PostalCd, ShptWt."
        )

        if skipped_sheets:

            st.warning(
                "Τα παρακάτω tabs παραλείφθηκαν επειδή "
                "δεν είχαν όλες τις απαραίτητες στήλες:"
            )
    ...
    st.success(
        f"Φορτώθηκαν {len(uploaded_files)} αρχεία "
        f"και {len(df_list)} tabs με δεδομένα."
    )
    ...
    # Αφαιρούμε γραμμές χωρίς διεύθυνση
    df = df.dropna(
        subset=["CneeAdd1"]
    ).copy()
    ...
    st.info(
        "⬆ Ανέβασε Excel αρχεία "
        "για να ξεκινήσει το report."
    )
